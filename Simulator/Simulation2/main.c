...
//		xSemaphoreGive(BinarySemaphore);
		...
		xTaskNotify(task2_handle,u1, eSetValueWithoutOverwrite);
		u1++;
		u1 %= 1000;
...
//		xSemaphoreTake(BinarySemaphore, 0);
		/* Take semaphore */
		u2[0] = ulTaskNotifyTake(pdFALSE, portMAX_DELAY);
		xTaskNotifyWait(0,0,&u2[1],portMAX_DELAY);
		/* Print out the name of this task. */
		printf("Present semaphore value: %d\n", u2[0]);
